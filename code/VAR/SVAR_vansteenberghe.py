...
resultpop.summary()
resultGDP.summary()
# indeed, as no eigenvalue greater or equal to 1, the coefficients are the same as below for the VAR

# Collect residuals
resid = pd.concat(
    [resultpop.resid.rename("e_pop"),
     resultGDP.resid.rename("e_gdp")],
    axis=1
).dropna()
...
cov_mat = np.cov(resid["e_pop"], resid["e_gdp"], ddof=1)
cov_offdiag = cov_mat[0, 1]

# Test H0: Cov = 0
# Equivalent to testing correlation = 0
r = np.corrcoef(resid["e_pop"], resid["e_gdp"])[0, 1]
t_stat = r * np.sqrt((n - 2) / (1 - r**2))
pval = 2 * (1 - stats.t.cdf(abs(t_stat), df=n-2))

print("Residual covariance matrix:\n", cov_mat)
print(f"Off-diagonal covariance = {cov_offdiag:.6f}")
print(f"Test H0: Cov = 0  (via correlation test)")
print(f"t-stat = {t_stat:.4f}, p-value = {pval:.4g}, n = {n}")
# => The residual covariance matrix is approximately diagonal

del dfallx2['GDP1'], dfallx2['Population1']

# VAR estimation (with OLS in fact)
# create a VAR
...
Omega = np.cov(U.T, ddof=1)

cov_offdiag = Omega[0, 1]

# Correlation (used for statistical test)
r = np.corrcoef(U.iloc[:, 0], U.iloc[:, 1])[0, 1]

# Test H0: Cov = 0  <=>  Corr = 0
t_stat = r * np.sqrt((T - 2) / (1 - r**2))
pval = 2 * (1 - stats.t.cdf(abs(t_stat), df=T - 2))

print("Estimated Omega (covariance matrix):\n", Omega)
print(f"\nOff-diagonal covariance = {cov_offdiag:.6f}")
print(f"Correlation = {r:.6f}")
print(f"Test H0: Cov = 0")
print(f"t-stat = {t_stat:.4f}, p-value = {pval:.4g}, T = {T}")
...
if ploton:
    result_varx.irf().plot(orth=True)
